<template>
  <div style="margin:50px">
    ...
    <v-layout row wrap>
      ...
      <v-flex v-for="(item,i) in items" :key="`4${i}`" xs4 sm4 md4 lg4 xl4 style="margin:0 auto;">        
...
export default {
    ...
    created(){        
        this.loading = true;
        http.axios.get(`/api/v1/myLikeLecture?page=${this.page}&itemsperpage=${this.itemsperpage}`).then(({data}) => {
            this.page++;
    ...
    methods: {
        handleScroll(){
            if($(document).scrollTop() + $(document)[0].scrollingElement.clientHeight + 100 >= $(document).height()){
                if(!this.loading){
                    this.loading = true;
                    http.axios.get(`/api/v1/myLikeLecture?page=${this.page}&itemsperpage=${this.itemsperpage}`)
                        .then(({data}) => {
                            this.page++;
