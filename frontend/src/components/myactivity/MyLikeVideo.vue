<template>
  <div style="margin:50px">
    ...
    <v-layout row wrap>
      ...
      <v-flex v-for="(item,i) in items" :key="`4${i}`" xs7 sm7 md7 lg7 xl7>        
...
export default {
    ...
    created(){        
        this.loading = true;
        http.axios.get(`/api/v1/myLikeVideo?page=${this.page}&itemsperpage=${this.itemsperpage}`).then(({data}) => {
            this.page++;
    ...
    methods: {
        handleScroll(){
            if($(document).scrollTop() + $(document)[0].scrollingElement.clientHeight + 100 >= $(document).height()){
                if(!this.loading){
                    this.loading = true;
                    http.axios.get(`/api/v1/myLikeVideo?page=${this.page}&itemsperpage=${this.itemsperpage}`)
                        .then(({data}) => {
                            this.page++;
